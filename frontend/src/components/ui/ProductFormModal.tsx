...
import { X, Upload, Save, Plus, Trash2, Check } from 'lucide-react';
...
export default function ProductFormModal({ isOpen, onClose, brand, product }: ProductFormModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => {
      ...
      return fetchApi(url, {
        method,
        body: JSON.stringify({ ...data, brand: dbBrand, price: parseFloat(data.price) })
      });
    },
    onSuccess: () => {
          ...
          <div className="flex items-center justify-between p-8 border-b border-white/5">
            <div>
              ...
              <p className="text-white/40 text-xs uppercase tracking-widest mt-1">
                {brand === 'salatto' ? 'DySalatto Mutfağı' : 'DyCake Atölyesi'}
              </p>
            </div>
            <button 
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <input 
                    ...
                    className={cn("w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white focus:outline-none transition-all", accentBorder)}
                    placeholder="Örn: Protein Kâsesi"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <select 
                    ...
                    className={cn("w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white focus:outline-none transition-all appearance-none", accentBorder)}
                    required
                  >
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-[10px] font-black tracking-widest text-white/40 uppercase mb-3">Görsel URL</label>
                    <input 
                      type="text"
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className={cn("w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white focus:outline-none transition-all min-h-[120px] resize-none", accentBorder)}
                    placeholder="Ürün içeriği ve detayları..."
                  />
                </div>
              </div>

              {/* Right Column: Traits & Ingredients */}
              <div className="space-y-8">
                {/* Traits (Filtreler) */}
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                    {ingredients.map((ing: any) => (
                      <button
                        ...
                        className={cn(
                          "px-3 py-2 rounded-lg text-[10px] font-bold border transition-all flex items-center gap-2",
            ...
            <div className="mt-12 pt-8 border-t border-white/5 flex justify-end gap-4">
               <button 
                type="button"
                onClick={onClose}
                className="px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-all"
               >
                 İPTAL
               </button>
               <button 
                ...
                className={cn("px-12 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-2xl transition-all flex items-center gap-3", accentColor, mutation.isPending && "opacity-50")}
               >
                 {mutation.isPending ? 'KAYDEDİLİYOR...' : (
                   <>
